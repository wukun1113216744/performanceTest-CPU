...
static __attribute__((noinline)) uint64_t rdtsc_overhead_func(uint64_t dummy) { return dummy; }

uint64_t global_rdtsc_overhead = UINT64_MAX;
...
static const long NSEC_PER_SEC = 1000000000;  // 10^9
...
#define BEST_TIME(test, expected, pre, repeat, size, verbose)                                      \
  do {                                                                                             \
    ...
    for (int i = 0; i < repeat; i++) {                                                             \
      ...
      diff = (end.tv_sec - start.tv_sec) * NSEC_PER_SEC + end.tv_nsec - start.tv_nsec;             \
      if (i == 0 || diff < min_diff) {                                                             \
        min_diff = diff;                                                                           \
      }                                                                                            \
      sum_diff += diff;                                                                            \
    }                                                                                              \
    float nsec_per_op = min_diff / (double)size;                                                   \
    float avg_nsec_per_op = sum_diff / ((double)size * repeat);                                    \
    if (verbose) {                                                                                 \
      printf(" %.2f nanoseconds per operation (best) ", nsec_per_op);                              \
      printf("\t%.2f nanoseconds per operation (avg)\n", avg_nsec_per_op);                         \
    } else {                                                                                       \
      printf(" %.2f ", nsec_per_op);                                                               \
    }                                                                                              \
    fflush(NULL);                                                                                  \
  } while (0)
